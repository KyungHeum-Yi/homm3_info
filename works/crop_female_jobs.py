...
raw_dir = r"C:\SourceCode\homm3_info\images\raw_data\영웅직업\여"
...
for job_id, (name, filename) in mapping.items():
    ...
    print(f"[OK] Cropped {name} (여) -> {dst_name} ({cropped.size})")
    success_count += 1

print(f"\nTotal cropped female images: {success_count}/{len(mapping)}")
